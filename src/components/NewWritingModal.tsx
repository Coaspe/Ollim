...
const NewWritingModal: React.FC<NewWritingProps> = ({ setNewWritingModalOpen }) => {
    ...
    const [firstCollectionElementError, setFirstCollectionElementError] = useState(false)
    const [title, setTitle] = useState("")
    const [titleError, setTitleError] = useState(false)
    const [synopsis, setSynopsis] = useState("")
    const [disclosure, setDisclosure] = useState<disclosure>("PUBLIC")
    const [isCollection, setIsCollection] = useState(false)
    const [firstCollectionElementTitle, setFirstCollectionElementTitle] = useState("")
    ...
    return (
        <motion.div onClick={() => {setNewWritingModalOpen(false)}} className="font-noto flex items-center justify-center z-20 fixed w-full h-full" animate={{ backgroundColor: ["hsla(0, 0%, 0%, 0)", "hsla(0, 0%, 0%, 0.8)"] }} transition={{ duration: 0.2, ease: "easeIn" }}>
            ...
            {page === "MAIN" && userInfo &&
            <div onClick={(e) => { e.stopPropagation() }} style={{backgroundColor: "#faf6f5"}} className="relative w-1/2 h-3/4 flex flex-col items-center justify-center rounded-xl">
                ...
                <motion.svg whileHover={{ y: "-10%" }} x="0px" y="0px" className={`w-8 absolute top-5 right-5 cursor-pointer`}
                    onClick={() => {
                        ...
                        !isCollection ? !titleError && !genrnError && setPage("DIAGRAM") : !titleError && !genrnError && !firstCollectionElementError && setPage("DIAGRAM")
                        }
                    }
                    viewBox="0 0 50 50">
                ...
                <div className="w-3/4 h-5/6 flex flex-col items-center justify-between">
                    ...
                    <div className="flex w-full h-1/3 items-center justify-between">
                            ...
                            <div className="flex justify-between items-center mt-5 w-full">
                                <Tooltip title="소설" placement="top" arrow>
                                    <span
                                        ...
                                        style={{fontSize:"1.5rem",borderColor: "#e4d0ca"}} className={`material-icons cursor-pointer border py-2 px-2 rounded-full hover:text-slate-500 hover:bg-hoverBGColor ${genrn === "NOVEL" ? "text-slate-700 bg-genreSelectedBG" : "text-slate-400"}`}>
                                        menu_book
                                    </span>
                                </Tooltip>
                                <Tooltip title="시" placement="top" arrow>
                                    <span onClick={() => {
                                            setGenrn("POEM")
                                            handleGenrnError()
                                        }} style={{fontSize:"1.5rem", borderColor: "#e4d0ca"}} className={`material-icons cursor-pointer border py-2 px-2 rounded-full hover:text-slate-500 hover:bg-hoverBGColor ${genrn === "POEM" ? "text-slate-700 bg-genreSelectedBG" : "text-slate-400"}`}>
                                        history_edu
                                    </span>
                                </Tooltip>
                                <Tooltip title="시나리오" placement="top" arrow>
                                    ...
                                    onClick={() => {
                                        ...
                                        className={`material-icons cursor-pointer border py-2 px-2 rounded-full hover:text-slate-500 hover:bg-hoverBGColor ${genrn === "SCENARIO" ? "text-slate-700 bg-genreSelectedBG" : "text-slate-400"}`}>
                                    adf_scanner
                                    </span>
                                </Tooltip>
                        ...
                        <div className="flex flex-col items-start w-1/4 h-full">
                            <div className="flex items-center justify-between">
                                <span className="text-xl font-bold">{genrn !== "POEM" ? "장 구분" : "시 집"}</span>
                            </div>
                            <div className="flex items-center mt-5 w-full justify-between">
                                ...
                                style={{fontSize: "1.5rem", borderColor: "#e4d0ca"}} className={`border rounded-full block py-2 px-2 cursor-pointer font-bold material-icons hover:text-slate-500 ${isCollection === true ? "text-slate-700 bg-genreSelectedBG" : "text-slate-400"}`}>
                                    done
                                </span>
                                <span                                
                                onClick={() => {
                                ...
                                style={{fontSize: "1.5rem", borderColor: "#e4d0ca"}} className={`border rounded-full block py-2 px-2 cursor-pointer font-bold material-icons hover:text-slate-500 ${isCollection === false ? "text-slate-700 bg-genreSelectedBG" : "text-slate-400"}`}>
                                    close
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-start w-1/4 h-full">
                            ...
                            <div className="w-full flex items-center mt-5 justify-between">
                                <Tooltip title="모두" placement="top" arrow>
                                    <button style={{ borderColor: "#e4d0ca" }} className={`group text-md font-bold border py-2 px-2 rounded-full hover:bg-hoverBGColor ${disclosure === "PUBLIC" && "bg-genreSelectedBG"}`} onClick={() => { setDisclosure("PUBLIC") }}>
                                        <svg className={`w-6 fill-slate-400 group-hover:fill-slate-500 ${disclosure === "PUBLIC" && "fill-slate-700"}`} viewBox="0 0 16 16">
                                ...
                                <Tooltip title="팔로워" placement="top" arrow>
                                    <button style={{ borderColor: "#e4d0ca" }} className={`group text-md font-bold border py-2 px-2 rounded-full hover:bg-hoverBGColor ${disclosure === "FOLLOWERS" && "bg-genreSelectedBG"}`} onClick={() => { setDisclosure("FOLLOWERS") }}>
                                        <svg x="0px" y="0px"
                                ...
                                <Tooltip title="비공개" placement="top" arrow>
                                    <button style={{ borderColor: "#e4d0ca" }} className={`group text-md font-bold border py-2 px-2 rounded-full hover:bg-hoverBGColor ${disclosure === "PRIVATE" && "bg-genreSelectedBG"}`} onClick={() => { setDisclosure("PRIVATE") }}>
                                        <svg className={`w-6 fill-slate-400 group-hover:fill-slate-500 ${disclosure === "PRIVATE" && "fill-slate-700"}`} viewBox="0 0 24 24">
                                            <g>
                    ...
                    <div className="flex w-full h-1/3 items-center justify-between">
                        ...
                        <AnimatePresence>
                            {isCollection && 
                            <motion.div animate={{y:["-5%", "0%"], opacity: ["0%", "100%"]}} exit={{y:["0%", "-5%"], opacity: ["100%", "0%"]}} transition={{duration: 0.1}} className="flex flex-col items-start w-5/12 h-full">
                                <div className="flex items-center">
                                    <span className="text-xl font-bold">{genrn !== "POEM" ? "첫 장의 제목" : "첫 시의 제목"}</span>
                                    {firstCollectionElementError && <motion.span className="text-sm ml-2 font-bold text-red-400" animate={{ opacity: [0, 1] }} transition={{ duration: 0.1 }}>제목을 입력해주세요!</motion.span>}
                                </div>
                                <input style={{borderColor: "#e4d0ca"}} className="text-lg font-md border-2 py-2 px-3 mt-5 rounded-xl w-full placeholder:italic"
                                    placeholder="제목을 입력해주세요."
                                    type="text"
                    ...
                    <div className="flex flex-col items-start w-full h-1/3">
                        ...
                        <textarea style={{borderColor: "#e4d0ca"}} className="resize-none border-2 pt-2 px-3 mt-5 rounded-xl h-full w-full italic" placeholder={genrn === 'POEM' ? "전시될 여는 말을 서술해주세요." : "전시될 시놉시스를 간략하게 서술해주세요."} value={synopsis} spellCheck="false" onChange={(e) => { setSynopsis(e.target.value) }} />
                    </div>
                </div>
            </div>}
